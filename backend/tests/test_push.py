...
def test_token_requires_auth(tmp_path):
    ...
    assert client.post("/api/push/token", json={"token": "t1", "platform": "android"}).status_code == 401


def test_registers_token(tmp_path):
    client, h = _authed_client(tmp_path)
    r = client.post("/api/push/token", json={"token": "t1", "platform": "android"}, headers=h)
...
def test_delete_token(tmp_path):
    client, h = _authed_client(tmp_path)
    client.post("/api/push/token", json={"token": "t-rm", "platform": "android"}, headers=h)
    r = client.delete("/api/push/token/t-rm", headers=h)
    assert r.status_code == 200 and r.json()["removed"] == 1
    store = UserStore(client.app.state.db)
    store.init_schema()
    assert store.conn.execute("SELECT COUNT(*) FROM push_tokens WHERE token='t-rm'").fetchone()[0] == 0
