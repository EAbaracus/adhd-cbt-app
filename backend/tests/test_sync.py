...
def test_backup_requires_auth(tmp_path):
    ...
    assert (
        client.put("/api/sync/backup", json={"kind": "forms", "items": {}}).status_code
        == 401
    )
...
def test_backup_upsert_idempotent(tmp_path):
    ...
    body = {
        "kind": "forms",
        "items": {
            "f1": {"payload": {"v": 1}, "updated_at": "2026-08-15T10:00:00+00:00"}
        },
    }
...
def test_backup_updates_existing_key(tmp_path):
    ...
    client.put(
        ...
        json={
            "kind": "forms",
            "items": {"f1": {"payload": {"v": 1}, "updated_at": "t1"}},
        },
        headers=h,
    )
    r = client.put(
        ...
        json={
            "kind": "forms",
            "items": {"f1": {"payload": {"v": 2}, "updated_at": "t2"}},
        },
        headers=h,
    )
...
def test_snapshot_returns_full_state(tmp_path):
    ...
    client.put(
        ...
        json={
            "kind": "logs",
            "items": {"l1": {"payload": {"m": "x"}, "updated_at": "t2"}},
        },
        headers=h,
    )
...
def test_per_user_isolation(tmp_path):
    ...
    client.put(
        "/api/sync/backup",
        json={"kind": "forms", "items": {"f1": {"payload": {}, "updated_at": "t"}}},
        headers=h,
    )
